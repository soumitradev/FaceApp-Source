...
class LoaderManagerImpl extends LoaderManager {
    ...
    static class LoaderViewModel extends ViewModel {
        private static final Factory FACTORY = new C06891();
        private SparseArrayCompat<LoaderInfo> mLoaders = new SparseArrayCompat();

        /* renamed from: android.support.v4.app.LoaderManagerImpl$LoaderViewModel$1 */
        static class C06891 implements Factory {
            C06891() {
            }
        ...
        protected void onCleared() {
            ...
            int size = this.mLoaders.size();
            for (int index = 0; index < size; index++) {
                ((LoaderInfo) this.mLoaders.valueAt(index)).destroy();
            }
            this.mLoaders.clear();
        }
    ...
    @MainThread
    @NonNull
    private <D> Loader<D> createAndInstallLoader(int id, @Nullable Bundle args, @NonNull LoaderCallbacks<D> callback) {
        ...
        try {
            ...
            if (!loader.getClass().isMemberClass() || Modifier.isStatic(loader.getClass().getModifiers())) {
                loaderInfo = new LoaderInfo(id, args, loader);
                if (DEBUG) {
                    String str = TAG;
                    stringBuilder = new StringBuilder();
