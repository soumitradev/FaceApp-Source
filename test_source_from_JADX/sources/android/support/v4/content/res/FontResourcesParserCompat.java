...
import android.support.compat.C0000R;
...
public class FontResourcesParserCompat {
    ...
    private static FamilyResourceEntry readFamily(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        TypedArray array = resources.obtainAttributes(Xml.asAttributeSet(parser), C0000R.styleable.FontFamily);
        String authority = array.getString(C0000R.styleable.FontFamily_fontProviderAuthority);
        String providerPackage = array.getString(C0000R.styleable.FontFamily_fontProviderPackage);
        String query = array.getString(C0000R.styleable.FontFamily_fontProviderQuery);
        int certsId = array.getResourceId(C0000R.styleable.FontFamily_fontProviderCerts, 0);
        int strategy = array.getInteger(C0000R.styleable.FontFamily_fontProviderFetchStrategy, 1);
        int timeoutMs = array.getInteger(C0000R.styleable.FontFamily_fontProviderFetchTimeout, 500);
        array.recycle();
    ...
    private static FontFileResourceEntry readFont(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        TypedArray array = resources.obtainAttributes(Xml.asAttributeSet(parser), C0000R.styleable.FontFamilyFont);
        int weight = array.getInt(array.hasValue(C0000R.styleable.FontFamilyFont_fontWeight) ? C0000R.styleable.FontFamilyFont_fontWeight : C0000R.styleable.FontFamilyFont_android_fontWeight, 400);
        boolean z = true;
        if (1 != array.getInt(array.hasValue(C0000R.styleable.FontFamilyFont_fontStyle) ? C0000R.styleable.FontFamilyFont_fontStyle : C0000R.styleable.FontFamilyFont_android_fontStyle, 0)) {
            z = false;
        }
        boolean isItalic = z;
        int resourceAttr = array.hasValue(C0000R.styleable.FontFamilyFont_font) ? C0000R.styleable.FontFamilyFont_font : C0000R.styleable.FontFamilyFont_android_font;
