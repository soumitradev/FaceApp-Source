...
class TypefaceCompatBaseImpl implements TypefaceCompat$TypefaceCompatImpl {
    ...
    class C06911 implements StyleExtractor<FontInfo> {
        C06911() {
        }
    ...
    class C06922 implements StyleExtractor<FontFileResourceEntry> {
        C06922() {
        }
    ...
    private static <T> T findBestFont(T[] fonts, int style, StyleExtractor<T> extractor) {
        int targetWeight = (style & 1) == 0 ? 400 : BrickValues.DRONE_VERTICAL_INDOOR;
    ...
    protected FontInfo findBestInfo(FontInfo[] fonts, int style) {
        return (FontInfo) findBestFont(fonts, style, new C06911());
    }
    ...
    private FontFileResourceEntry findBestEntry(FontFamilyFilesResourceEntry entry, int style) {
        return (FontFileResourceEntry) findBestFont(entry.getEntries(), style, new C06922());
    }
