...
class TwilightCalculator {
    ...
    public void calculateTwilight(long time, double latitude, double longitude) {
        ...
        trueAnomaly = 0.01745329238474369d * latitude;
        solarLng = (Math.sin(-0.10471975803375244d) - (Math.sin(trueAnomaly) * Math.sin(solarDec))) / (Math.cos(trueAnomaly) * Math.cos(solarDec));
        if (solarLng >= 1.0d) {
            twilightCalculator.state = 1;
            twilightCalculator.sunset = -1;
            twilightCalculator.sunrise = -1;
        } else if (solarLng <= -1.0d) {
            twilightCalculator.state = 0;
            twilightCalculator.sunset = -1;
            twilightCalculator.sunrise = -1;
        } else {
            daysSince2000 = (float) (Math.acos(solarLng) / 6.283185307179586d);
