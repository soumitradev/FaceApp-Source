...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new C02591();

    /* renamed from: android.support.v7.util.DiffUtil$1 */
    static class C02591 implements Comparator<Snake> {
        C02591() {
        }

        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f23x - o2.f23x;
            return cmpX == 0 ? o1.f24y - o2.f24y : cmpX;
        }
    }
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f23x != 0 || firstSnake.f24y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f23x + snake.size;
                int endY = snake.f24y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f23x + j;
                    int newItemPos = snake.f24y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            while (true) {
                ...
                if (snakeIndex2 >= 0) {
                    ...
                    int endX = snake.f23x + snakeSize;
                    int endY2 = snake.f24y + snakeSize;
                    if (endX < posOld2) {
                        endY = endY2;
                    ...
                    while (true) {
                        ...
                        if ((diffResult.mOldItemStatuses[snake.f23x + i] & 31) == 2) {
                            batchingCallback.onChanged(snake.f23x + i, 1, diffResult.mCallback.getChangePayload(snake.f23x + i, snake.f24y + i));
                        }
                        snakeSize = i - 1;
                    }
                    posOld2 = snake.f23x;
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (true) {
            ...
            if (snake != null) {
                ...
                snake.f23x += range.oldListStart;
                snake.f24y += range.newListStart;
                Range left = rangePool2.isEmpty() ? new Range() : (Range) rangePool2.remove(rangePool2.size() - 1);
                left.oldListStart = range.oldListStart;
                left.newListStart = range.newListStart;
                if (snake.reverse) {
                    left.oldListEnd = snake.f23x;
                    left.newListEnd = snake.f24y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f23x - 1;
                    left.newListEnd = snake.f24y;
                } else {
                    left.oldListEnd = snake.f23x;
                    left.newListEnd = snake.f24y - 1;
                }
                stack.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f23x + snake.size;
                    right.newListStart = snake.f24y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = (snake.f23x + snake.size) + 1;
                    right.newListStart = snake.f24y + snake.size;
                } else {
                    right.oldListStart = snake.f23x + snake.size;
                    right.newListStart = (snake.f24y + snake.size) + 1;
                }
                stack.add(right);
            } else {
