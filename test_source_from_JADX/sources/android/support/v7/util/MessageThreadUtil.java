...
class MessageThreadUtil<T> implements ThreadUtil<T> {
    ...
    public MainThreadCallback<T> getMainThreadProxy(final MainThreadCallback<T> callback) {
        return new MainThreadCallback<T>() {
            ...
            private Runnable mMainThreadRunnable = new C02601();
            ...
            class C02601 implements Runnable {
                ...
                public void run() {
                    SyncQueueItem msg = C07131.this.mQueue.next();
                    while (msg != null) {
                        switch (msg.what) {
                            case 1:
    ...
    public BackgroundCallback<T> getBackgroundProxy(final BackgroundCallback<T> callback) {
        return new BackgroundCallback<T>() {
            ...
            private Runnable mBackgroundRunnable = new C02611();
            ...
            class C02611 implements Runnable {
                ...
                public void run() {
                    while (true) {
                        SyncQueueItem msg = C07142.this.mQueue.next();
                        if (msg != null) {
                            switch (msg.what) {
                                case 1:
                                    C07142.this.mQueue.removeMessages(1);
                                    callback.refresh(msg.arg1);
                                    break;
                                case 2:
                                    C07142.this.mQueue.removeMessages(2);
                                    C07142.this.mQueue.removeMessages(3);
                        ...
                        C07142.this.mBackgroundRunning.set(false);
                        return;
                    }
                }
            }
