...
public class BrowserActionsIntent {
    ...
    public static final class Builder {
        ...
        public Builder setCustomItems(ArrayList<BrowserActionItem> items) {
            ...
            int i = 0;
            while (i < items.size()) {
                if (!TextUtils.isEmpty(((BrowserActionItem) items.get(i)).getTitle())) {
                    if (((BrowserActionItem) items.get(i)).getAction() != null) {
                        this.mMenuItems.add(getBundleFromItem((BrowserActionItem) items.get(i)));
                        i++;
                    }
                }
    ...
    public static void openBrowserAction(Context context, Uri uri, int type, ArrayList<BrowserActionItem> items, PendingIntent pendingIntent) {
        launchIntent(context, new Builder(context, uri).setUrlType(type).setCustomItems((ArrayList) items).setOnItemSelectedAction(pendingIntent).build().getIntent());
    }
                ...
                if (handlers.size() == 1) {
                    ...
                    if (defaultHandler != null) {
                        ...
                        while (i < handlers.size()) {
                            if (defaultPackageName.equals(((ResolveInfo) handlers.get(i)).activityInfo.packageName)) {
