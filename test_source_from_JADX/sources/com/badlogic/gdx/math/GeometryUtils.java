...
public final class GeometryUtils {
    ...
    public static Vector2 toBarycoord(Vector2 p, Vector2 a, Vector2 b, Vector2 c, Vector2 barycentricOut) {
        ...
        barycentricOut.f16x = ((d11 * d20) - (d01 * d21)) / denom;
        barycentricOut.f17y = ((d00 * d21) - (d01 * d20)) / denom;
        return barycentricOut;
    }

    public static boolean barycoordInsideTriangle(Vector2 barycentric) {
        return barycentric.f16x >= 0.0f && barycentric.f17y >= 0.0f && barycentric.f16x + barycentric.f17y <= 1.0f;
    }

    public static Vector2 fromBarycoord(Vector2 barycentric, Vector2 a, Vector2 b, Vector2 c, Vector2 interpolatedOut) {
        float u = (1.0f - barycentric.f16x) - barycentric.f17y;
        interpolatedOut.f16x = ((a.f16x * u) + (barycentric.f16x * b.f16x)) + (barycentric.f17y * c.f16x);
        interpolatedOut.f17y = ((a.f17y * u) + (barycentric.f16x * b.f17y)) + (barycentric.f17y * c.f17y);
        return interpolatedOut;
    }

    public static float fromBarycoord(Vector2 barycentric, float a, float b, float c) {
        return ((((1.0f - barycentric.f16x) - barycentric.f17y) * a) + (barycentric.f16x * b)) + (barycentric.f17y * c);
    }
    ...
    public static Vector2 triangleCircumcenter(float x1, float y1, float x2, float y2, float x3, float y3, Vector2 circumcenter) {
        ...
        vector2.set((((sqr1 * dy32) + (sqr2 * dy13)) + (sqr3 * dy21)) / det, (-(((sqr1 * dx32) + (sqr2 * dx13)) + (sqr3 * dx21))) / det);
        return vector2;
    }
    ...
    public static Vector2 quadrilateralCentroid(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, Vector2 centroid) {
        ...
        centroid.f16x = avgX1 - ((avgX1 - (((x1 + x4) + x3) / 3.0f)) / 2.0f);
        centroid.f17y = avgY1 - ((avgY1 - avgY2) / 2.0f);
        return centroid;
    }
    ...
    public static Vector2 polygonCentroid(float[] polygon, int offset, int count, Vector2 centroid) {
        ...
        if (signedArea == 0.0f) {
            ...
            signedArea *= 0.5f;
            centroid.f16x = x / (signedArea * 6.0f);
            centroid.f17y = y / (6.0f * signedArea);
        }
        return centroid;
    }
    ...
    private static boolean areVerticesClockwise(float[] polygon, int offset, int count) {
        ...
        float p1x2 = polygon[count - 2];
        float p1y2 = polygon[count - 1];
        if (((p1x2 * polygon[1]) + area) - (polygon[0] * p1y2) < 0.0f) {
            z = true;
        }
        return z;
    }
