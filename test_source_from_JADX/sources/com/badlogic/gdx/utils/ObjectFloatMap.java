...
public class ObjectFloatMap<K> implements Iterable<Entry<K>> {
    ...
    public void put(K key, float value) {
        ...
        K[] keyTable = objectFloatMap.keyTable;
        int hashCode = key.hashCode();
        int index1 = hashCode & objectFloatMap.mask;
        K key1 = keyTable[index1];
        if (k.equals(key1)) {
            objectFloatMap.valueTable[index1] = value;
            return;
        }
        int index2 = hash2(hashCode);
        K key2 = keyTable[index2];
        if (k.equals(key2)) {
            objectFloatMap.valueTable[index2] = value;
            return;
        }
        int index3 = hash3(hashCode);
        K key3 = keyTable[index3];
        if (k.equals(key3)) {
            objectFloatMap.valueTable[index3] = value;
            return;
        }
        int i = objectFloatMap.capacity;
        int n = objectFloatMap.stashSize + i;
        while (i < n) {
            if (k.equals(keyTable[i])) {
                objectFloatMap.valueTable[i] = value;
                return;
            }
            i++;
        }
        if (key1 == null) {
            keyTable[index1] = k;
            objectFloatMap.valueTable[index1] = value;
            i = objectFloatMap.size;
            objectFloatMap.size = i + 1;
            if (i >= objectFloatMap.threshold) {
                resize(objectFloatMap.capacity << 1);
            }
        } else if (key2 == null) {
            keyTable[index2] = k;
            objectFloatMap.valueTable[index2] = value;
