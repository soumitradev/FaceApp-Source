...
public class Beta extends Kit<Boolean> implements DeviceIdentifierProvider {
    ...
    private String getBetaDeviceToken(Context context, String installerPackageName) {
        ...
        String token = null;
        try {
            cachedToken = (String) this.deviceTokenCache.get(context, this.deviceTokenLoader);
        ...
        stringBuilder.append(TextUtils.isEmpty(token) ^ 1);
        logger.d(cachedToken, stringBuilder.toString());
        return token;
    }
    ...
    private BuildProperties loadBuildProperties(Context context) {
        ...
        try {
            ...
            if (buildPropsStream != null) {
                ...
                String str = TAG;
                StringBuilder stringBuilder = new StringBuilder();
                stringBuilder.append(buildProps.packageName);
                stringBuilder.append(" build properties: ");
                stringBuilder.append(buildProps.versionName);
                stringBuilder.append(" (");
                stringBuilder.append(buildProps.versionCode);
                stringBuilder.append(") - ");
                stringBuilder.append(buildProps.buildId);
