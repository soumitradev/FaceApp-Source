...
public class LoginButton extends FacebookButtonBase {
    ...
    protected class LoginClickListener implements OnClickListener {
        ...
        protected void performLogout(Context context) {
            ...
            if (LoginButton.this.confirmLogout) {
                String message;
                String logout = LoginButton.this.getResources().getString(C0410R.string.com_facebook_loginview_log_out_action);
                String cancel = LoginButton.this.getResources().getString(C0410R.string.com_facebook_loginview_cancel_action);
                Profile profile = Profile.getCurrentProfile();
                if (profile == null || profile.getName() == null) {
                    message = LoginButton.this.getResources().getString(C0410R.string.com_facebook_loginview_logged_in_using_facebook);
                } else {
                    message = String.format(LoginButton.this.getResources().getString(C0410R.string.com_facebook_loginview_logged_in_as), new Object[]{profile.getName()});
                }
    ...
    class C08462 extends AccessTokenTracker {
        C08462() {
        }
    ...
    private void checkToolTipSettings() {
        switch (this.toolTipMode) {
            case AUTOMATIC:
                ...
                return;
            case DISPLAY_ALWAYS:
                displayToolTip(getResources().getString(C0410R.string.com_facebook_tooltip_default));
                return;
            default:
                return;
        }
    ...
    protected void configureButton(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (isInEditMode()) {
            setBackgroundColor(getResources().getColor(C0410R.color.com_facebook_blue));
            this.loginText = "Log in with Facebook";
        } else {
            this.accessTokenTracker = new C08462();
        }
        setButtonText();
    }
    ...
    protected int getDefaultStyleResource() {
        return C0410R.style.com_facebook_loginview_default_style;
    }

    private void parseLoginButtonAttributes(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        this.toolTipMode = ToolTipMode.DEFAULT;
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, C0410R.styleable.com_facebook_login_view, defStyleAttr, defStyleRes);
        try {
            this.confirmLogout = a.getBoolean(C0410R.styleable.com_facebook_login_view_com_facebook_confirm_logout, true);
            this.loginText = a.getString(C0410R.styleable.com_facebook_login_view_com_facebook_login_text);
            this.logoutText = a.getString(C0410R.styleable.com_facebook_login_view_com_facebook_logout_text);
            this.toolTipMode = ToolTipMode.fromInt(a.getInt(C0410R.styleable.com_facebook_login_view_com_facebook_tooltip_mode, ToolTipMode.DEFAULT.getValue()));
        } finally {
            a.recycle();
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (text == null) {
            text = resources.getString(C0410R.string.com_facebook_loginview_log_in_button_long);
            logInWidth = measureButtonWidth(text);
            if (resolveSize(logInWidth, widthMeasureSpec) < logInWidth) {
                text = resources.getString(C0410R.string.com_facebook_loginview_log_in_button);
            }
        }
        logInWidth = measureButtonWidth(text);
        text = this.logoutText;
        if (text == null) {
            text = resources.getString(C0410R.string.com_facebook_loginview_log_out_button);
        }
    ...
    private void setButtonText() {
        ...
        if (!isInEditMode() && AccessToken.getCurrentAccessToken() != null) {
            setText(this.logoutText != null ? this.logoutText : resources.getString(C0410R.string.com_facebook_loginview_log_out_button));
        } else if (this.loginText != null) {
            setText(this.loginText);
        } else {
            String text = resources.getString(C0410R.string.com_facebook_loginview_log_in_button_long);
            int width = getWidth();
