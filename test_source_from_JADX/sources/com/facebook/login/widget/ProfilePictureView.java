...
public class ProfilePictureView extends FrameLayout {
    ...
    class C08471 implements Callback {
        C08471() {
        }
    ...
    private void parseAttributes(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, C0410R.styleable.com_facebook_profile_picture_view);
        setPresetSize(a.getInt(C0410R.styleable.com_facebook_profile_picture_view_com_facebook_preset_size, -1));
        this.isCropped = a.getBoolean(C0410R.styleable.com_facebook_profile_picture_view_com_facebook_is_cropped, true);
        a.recycle();
    }
    ...
    private void setBlankProfilePicture() {
        ...
        if (this.customizedDefaultProfilePicture == null) {
            setImageBitmap(BitmapFactory.decodeResource(getResources(), isCropped() ? C0410R.drawable.com_facebook_profile_picture_blank_square : C0410R.drawable.com_facebook_profile_picture_blank_portrait));
            return;
        }
    ...
    private void sendImageRequest(boolean allowCachedResponse) {
        ImageRequest request = new Builder(getContext(), ImageRequest.getProfilePictureUri(this.profileId, this.queryWidth, this.queryHeight)).setAllowCachedRedirects(allowCachedResponse).setCallerTag(this).setCallback(new C08471()).build();
        if (this.lastRequest != null) {
            ImageDownloader.cancelRequest(this.lastRequest);
        }
        ...
        if (response.getRequest() == this.lastRequest) {
            ...
            if (error != null) {
                ...
                if (listener != null) {
                    ...
                    listener.onError(new FacebookException(stringBuilder.toString(), error));
                } else {
    ...
    private boolean updateImageQueryParameters() {
        ...
        if (newWidthPx >= 1) {
            if (newHeightPx >= 1) {
                ...
                if (newWidthPx == this.queryWidth) {
                    if (newHeightPx == this.queryHeight) {
                        changed = false;
                    }
                }
    ...
    private int getPresetSizeInPixels(boolean forcePreset) {
        ...
        switch (this.presetSizeType) {
            case -4:
                dimensionId = C0410R.dimen.com_facebook_profilepictureview_preset_size_large;
                break;
            case -3:
                dimensionId = C0410R.dimen.com_facebook_profilepictureview_preset_size_normal;
                break;
            case -2:
                dimensionId = C0410R.dimen.com_facebook_profilepictureview_preset_size_small;
                break;
            case -1:
                if (forcePreset) {
                    dimensionId = C0410R.dimen.com_facebook_profilepictureview_preset_size_normal;
