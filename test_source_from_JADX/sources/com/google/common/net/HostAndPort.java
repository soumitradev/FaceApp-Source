...
public final class HostAndPort implements Serializable {
    ...
    public static HostAndPort fromParts(String host, int port) {
        ...
        Preconditions.checkArgument(parsedHost.hasPort() ^ true, "Host has a port: %s", host);
    ...
    public static HostAndPort fromString(String hostPortString) {
        ...
        if (!Strings.isNullOrEmpty(portString)) {
            Preconditions.checkArgument(portString.startsWith("+") ^ true, "Unparseable port number: %s", hostPortString);
            try {
    ...
    private static String[] getHostAndPortFromBracketedHost(String hostPortString) {
        Preconditions.checkArgument(hostPortString.charAt(0) == Constants.REMIX_URL_PREFIX_INDICATOR, "Bracketed host-port string must start with a bracket: %s", hostPortString);
