...
public final class InternetDomainName {
    ...
    private static boolean validatePart(String part, boolean isFinalPart) {
        if (part.length() >= 1) {
            if (part.length() <= 63) {
                if (PART_CHAR_MATCHER.matchesAllOf(CharMatcher.ASCII.retainFrom(part)) && !DASH_MATCHER.matches(part.charAt(0))) {
                    if (!DASH_MATCHER.matches(part.charAt(part.length() - 1))) {
