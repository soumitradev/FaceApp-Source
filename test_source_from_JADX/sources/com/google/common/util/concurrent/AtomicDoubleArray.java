...
public class AtomicDoubleArray implements Serializable {
    ...
    public final double getAndAdd(int i, double delta) {
        AtomicDoubleArray atomicDoubleArray = this;
        while (true) {
            int i2 = i;
            long current = atomicDoubleArray.longs.get(i2);
            double currentVal = Double.longBitsToDouble(current);
            double nextVal = currentVal + delta;
            if (atomicDoubleArray.longs.compareAndSet(i2, current, Double.doubleToRawLongBits(nextVal))) {
                return currentVal;
            }
        }
    }
    ...
    public String toString() {
        ...
        b.append(Constants.REMIX_URL_PREFIX_INDICATOR);
        int i = 0;
        while (true) {
            b.append(Double.longBitsToDouble(this.longs.get(i)));
            if (i == iMax) {
                b.append(Constants.REMIX_URL_SUFIX_INDICATOR);
                return b.toString();
            }
            b.append(Constants.REMIX_URL_SEPARATOR);
            b.append(' ');
            i++;
        }
    }
