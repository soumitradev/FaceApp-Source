...
public class JsonReader implements Closeable {
    ...
    static class C10801 extends JsonReaderInternalAccess {
        C10801() {
        }
    ...
    static {
        JsonReaderInternalAccess.INSTANCE = new C10801();
    }
            ...
            if (p2 + p == l) {
                ...
                if (fillBuffer(p + 1)) {
                    ...
                    if (last == 2 && fitsInLong) {
                        ...
                        r0.peekedLong = negative ? value2 : -value2;
                        r0.pos += p;
                        r0.peeked = 15;
                        return 15;
                    }
                                    ...
                                    if (last != 0) {
                                        if (last == 2) {
                                            if (value != 0) {
                                                ...
                                                if (value2 <= MIN_INCOMPLETE_INTEGER) {
                                                    if (value2 != MIN_INCOMPLETE_INTEGER || newValue >= value2) {
                                                        i2 = 0;
                                                    }
                                                }
                                                fitsInLong &= i2;
    ...
    private char readEscapeCharacter() throws IOException {
        if (this.pos != this.limit || fillBuffer(1)) {
            ...
            if (escaped == '\n') {
                this.lineNumber++;
                this.lineStart = this.pos;
            } else if (!(escaped == Typography.quote || escaped == '\'' || escaped == '/' || escaped == '\\')) {
                if (escaped == 'b') {
                    return '\b';
                }
                if (escaped == 'f') {
