        ...
        while (i < size) {
            ...
            while (true) {
                ...
                if (pos3 != value.length()) {
                    i2 = maxAgeSeconds;
                    if (value.charAt(pos3) != Constants.REMIX_URL_SEPARATOR) {
                        if (value.charAt(pos3) != Constants.REMIX_URL_REPLACE_SEPARATOR) {
                            pos3 = HeaderParser.skipWhitespace(value, pos3 + 1);
                            int parameterStart;
                            if (pos3 >= value.length() || value.charAt(pos3) != Typography.quote) {
                                parameterStart = pos3;
