...
class Huffman {
    ...
    void encode(byte[] data, OutputStream out) throws IOException {
        ...
        while (i < data.length) {
            ...
            while (n >= 8) {
                n -= 8;
                out.write((int) (current2 >> n));
            }
            i++;
            current = current2;
        }
        if (n > 0) {
            long current3 = (current << (8 - n)) | ((long) (255 >>> n));
            out.write((int) current3);
            current = current3;
        }
    }
