...
public class FileStoreImpl implements FileStore {
    ...
    public FileStoreImpl(Kit kit) {
        ...
        this.legacySupport = stringBuilder.toString();
    }
    ...
    File prepare(File file) {
        if (file != null) {
            if (!file.exists()) {
                if (!file.mkdirs()) {
                    Fabric.getLogger().mo4822w(Fabric.TAG, "Couldn't create file");
                }
            }
            return file;
        }
        Fabric.getLogger().mo4809d(Fabric.TAG, "Null File");
        return null;
    }
    ...
    boolean isExternalStorageAvailable() {
        ...
        Fabric.getLogger().mo4822w(Fabric.TAG, "External Storage is not mounted and/or writable\nHave you declared android.permission.WRITE_EXTERNAL_STORAGE in the manifest?");
        return false;
    }
}
