...
class DefaultSettingsController implements SettingsController {
    ...
    public SettingsData loadSettingsData(SettingsCacheBehavior cacheBehavior) {
        ...
        try {
            ...
            Fabric.getLogger().mo4812e(Fabric.TAG, LOAD_ERROR_MESSAGE, e);
        }
        return toReturn;
    }
                    ...
                    if (settingsData != null) {
                        ...
                        if (!SettingsCacheBehavior.IGNORE_CACHE_EXPIRATION.equals(cacheBehavior)) {
                            if (settingsData.isExpired(currentTimeMillis)) {
                                Fabric.getLogger().mo4809d(Fabric.TAG, "Cached settings have expired.");
                            }
                        }
                        toReturn = settingsData;
                        Fabric.getLogger().mo4809d(Fabric.TAG, "Returning cached settings.");
                    } else {
                        Fabric.getLogger().mo4812e(Fabric.TAG, "Failed to transform cached settings data.", null);
                    }
                } else {
                    Fabric.getLogger().mo4809d(Fabric.TAG, "No cached settings data found.");
                }
            }
        } catch (Exception e) {
            Fabric.getLogger().mo4812e(Fabric.TAG, "Failed to get cached settings", e);
        }
        return toReturn;
    }
