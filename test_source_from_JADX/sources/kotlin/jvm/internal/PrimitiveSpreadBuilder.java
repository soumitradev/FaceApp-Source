...
public abstract class PrimitiveSpreadBuilder<T> {
    ...
    protected final T toArray(@NotNull T values, @NotNull T result) {
        ...
        if (i >= 0) {
            ...
            dstIndex = 0;
            while (true) {
                Object spreadArgument = this.spreads[dstIndex];
                if (spreadArgument != null) {
                    if (copyValuesFrom < dstIndex) {
                        System.arraycopy(values, copyValuesFrom, result, dstIndex2, dstIndex - copyValuesFrom);
                        dstIndex2 += dstIndex - copyValuesFrom;
                    }
                    int spreadSize = getSize(spreadArgument);
                    System.arraycopy(spreadArgument, 0, result, dstIndex2, spreadSize);
                    dstIndex2 += spreadSize;
                    copyValuesFrom = dstIndex + 1;
                }
                if (dstIndex == i) {
                    break;
                }
