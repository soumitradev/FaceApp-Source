...
public abstract class ZipUtil {
    ...
    public static byte[] toDosTime(long t) {
        ...
        return ZipLong.getBytes((long) ((c.get(13) >> 1) | ((((((year - 1980) << 25) | ((c.get(2) + 1) << 21)) | (c.get(5) << 16)) | (c.get(11) << 11)) | (c.get(12) << 5))));
    }
    ...
    static long bigToLong(BigInteger big) {
        ...
        stringBuilder.append("The BigInteger cannot fit inside a 64 bit java long: [");
    ...
    static BigInteger longToBig(long l) {
        if (l < -2147483648L) {
            StringBuilder stringBuilder = new StringBuilder();
            stringBuilder.append("Negative longs < -2^31 not permitted: [");
    ...
    public static byte unsignedIntToSignedByte(int i) {
        ...
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder.append("Can only convert non-negative integers between [0,255] to byte: [");
    ...
    private static boolean supportsEncryptionOf(ZipArchiveEntry entry) {
        return entry.getGeneralPurposeBit().usesEncryption() ^ 1;
    }

    private static boolean supportsMethodOf(ZipArchiveEntry entry) {
        if (!(entry.getMethod() == 0 || entry.getMethod() == ZipMethod.UNSHRINKING.getCode() || entry.getMethod() == ZipMethod.IMPLODING.getCode())) {
