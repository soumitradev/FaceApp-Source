...
public class GoogleLogInTask extends AsyncTask<Void, Void, Boolean> {
    ...
    public void setOnGoogleServerLogInCompleteListener(OnGoogleServerLogInCompleteListener listener) {
        this.onGoogleServerLogInCompleteListener = listener;
    }

    protected void onPreExecute() {
        super.onPreExecute();
        if (this.context != null) {
            this.progressDialog = ProgressDialog.show(this.context, this.context.getString(R.string.please_wait), this.context.getString(R.string.loading_google_login));
        }
    }
    ...
    protected Boolean doInBackground(Void... arg0) {
        try {
            if (Utils.isNetworkAvailable(this.context)) {
                this.userSignedIn = ServerCalls.getInstance().googleLogin(this.mail, this.username, this.id, this.locale, this.context);
    ...
    protected void onPostExecute(Boolean success) {
        ...
        } else if ((success.booleanValue() || this.exception == null) && this.userSignedIn) {
            if (this.userSignedIn) {
                ToastUtil.showSuccess(this.context, (int) R.string.user_logged_in);
            }
            if (this.onGoogleServerLogInCompleteListener != null) {
                this.onGoogleServerLogInCompleteListener.onGoogleServerLogInComplete();
            }
        } else {
